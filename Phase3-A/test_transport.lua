...
test('full runtime: failed per-wheel getter cannot reuse previous wheel state',function()
    local f=fullRuntime(); f:runFrames(10)
    local original=f.cache['modules.tire_force'].getState
    f.cache['modules.tire_force'].getState=function(i)
        if i==0 then error('test wheel getter failure') end
        return original(i)
    end
    f:runFrames(3)
    equal(f.output.state.valid,false); equal(f.output.state.values,nil)
    assert(f.output.state.error_count>0 and f.output.state.failure:find('test wheel getter failure'))
end)
test('full runtime: failed Hub getter clears the previous output reference',function()
    local f=fullRuntime(); f:runFrames(10)
    f.hub.getState=function() error('test Hub getter failure') end
    f:runFrames(3)
    equal(f.output.state.valid,false); equal(f.bridge.state.sourceValid,false)
    assert(f.output.state.error_count>0 and f.output.state.failure:find('test Hub getter failure'))
end)
