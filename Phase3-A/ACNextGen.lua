...
local moduleDefs = {
    -- Phase 1 baseline modules are retained; Phase 3 adds bridge/verification modules.
    ...
    -- Output hub is deliberately last in the calculation chain.
    { name = "physics",            enabled = true, critical = true,  diagnostic = false },

    -- Phase 3 bridge is executed after the Physics Hub.
    { name = "physics_bridge",     enabled = true, critical = true,  diagnostic = false },

    -- Phase 3-A verifies the Worker transport result only.
    -- It never calculates physics and never injects force.
    { name = "worker_output",      enabled = true, critical = true,  diagnostic = true  },

    -- Observer is intentionally executed after Worker Output so the UI
    -- always reads the newest Phase 3-A verification state.
...
local workerOutputModule = nil
local physicsBridgeModule = nil
...
local function loadModules()
    modules = {}
    observerModule = nil
    workerOutputModule = nil
    physicsBridgeModule = nil
    ...
    for i = 1, #moduleDefs do
        ...
        if not entry.enabled then
            ...
            if entry.loaded then
                ...
                elseif entry.name == "worker_output" then
                    workerOutputModule = entry.module
                elseif entry.name == "physics_bridge" then
                    physicsBridgeModule = entry.module
                end
...
local function storeRuntime()
    ...
    safeStore("ngp_phase3_bridge", runtime.physicsBridge and 1 or 0)
    safeStore("ngp_phase3_worker_output", runtime.workerOutput and 1 or 0)
    safeStore("ngp_phase3a_observer", runtime.observerState and 1 or 0)
    safeStore("ngp_phase3a_status",
        runtime.workerOutput and tostring(runtime.workerOutput.status or "WAIT") or "WAIT")
    safeStore("ngp_phase3a_tick",
        runtime.workerOutput and tonumber(runtime.workerOutput.tick or 0) or 0)
    safeStore("ngp_phase3a_transfer_count",
        runtime.workerOutput and tonumber(runtime.workerOutput.transfer_count or 0) or 0)
    safeStore("ngp_phase3a_error_count",
        runtime.workerOutput and tonumber(runtime.workerOutput.error_count or 0) or 0)
    safeStore("ngp_phase3a_stale",
        runtime.workerOutput and (runtime.workerOutput.stale and 1 or 0) or 1)
    safeStore("ngp_phase3a_injection_enabled", 0)
    safeStore("ngp_phase3a_injection_applied", 0)
...
function update(dt)
    ...
    --========================================================
    -- Phase 3-A strict execution order
    --
    --   Calculation modules
    --        -> Physics Hub
    --        -> Physics Bridge
    ...
    --        -> Observer
    --
    -- Bridge/verification/observer are excluded from the generic
    -- scheduler so their data dependencies are deterministic.
    --========================================================

    for i = 1, #modules do
        local name = modules[i].name
        if name ~= "physics_bridge"
        and name ~= "worker_output"
        and name ~= "observer" then
            safeUpdate(modules[i], dt, car, profileNow)
        end
    end

    -- Physics Bridge: strictly downstream of Physics Hub.
    local bridgeEntry = nil
    for i = 1, #modules do
        if modules[i].name == "physics_bridge" then
            bridgeEntry = modules[i]
            break
        end
    end

    if bridgeEntry then
        safeUpdate(bridgeEntry, dt, car, false)
        runtime.physicsBridge = runtime.moduleStates.physics_bridge
    end

    -- Worker Output Verification: strictly after the bridge.
    local workerOutputEntry = nil
    for i = 1, #modules do
        if modules[i].name == "worker_output" then
            workerOutputEntry = modules[i]
            break
        end
    end

    if workerOutputEntry then
        safeUpdate(workerOutputEntry, dt, car, false)
        runtime.workerOutput = runtime.moduleStates.worker_output
    end

    -- Observer: strictly last so the UI reads the current frame's
    -- bridge and Worker Output verification state.
    local observerEntry = nil
    for i = 1, #modules do
        if modules[i].name == "observer" then
            observerEntry = modules[i]
            break
        end
    end

    if observerEntry then
        safeUpdate(observerEntry, dt, car, false)
        runtime.observerState = runtime.moduleStates.observer
    end
