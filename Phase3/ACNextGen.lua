...
local VERSION  = "V1.1 Phase 3 / Physics Bridge"
...
local function safeUpdate(entry, dt, car, profileNow)
    ...
    if ok then
        ...
        if entry.name == "physics" and type(entry.module.getState) == "function" then
        ...
        elseif entry.name == "physics_bridge" and type(entry.module.getState) == "function" then
            local bridgeOK, bridgeState = pcall(entry.module.getState)
            if bridgeOK and bridgeState then
                runtime.moduleStates[entry.name] = bridgeState
                runtime.physicsBridge = bridgeState
            end
        end
    else
...
function update(dt)
    ...
    -- Phase 3 bridge runs after Physics.lua so it is strictly downstream.
    -- The bridge itself is never allowed to become an upstream calculator.
